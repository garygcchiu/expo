...
open class JavaScriptRuntime: Equatable, @unchecked Sendable {
  ...
  public func eval(label: String? = nil, _ source: String) throws -> JavaScriptValue {
    ...
    let value = JavaScriptValue(self, jsiValue)

    if value.is("Error") {
      throw ScriptEvaluationError(message: value.getObject().getProperty("message").getString())
    }
    return value
  }
...
private func createFunctionClosure(runtime: JavaScriptRuntime, name: String, _ closure: @escaping JavaScriptRuntime.SyncFunctionClosure) -> expo.HostFunctionClosure {
