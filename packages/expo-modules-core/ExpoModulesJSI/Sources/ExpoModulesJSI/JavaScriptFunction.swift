...
public struct JavaScriptFunction: JavaScriptType, ~Copyable {
   ...
   */
  @discardableResult
  public func call(this: borrowing JavaScriptObject, arguments: consuming JSValuesBuffer? = nil) -> JavaScriptValue {
   ...
   */
  @discardableResult
  public func call(arguments: consuming JSValuesBuffer? = nil) -> JavaScriptValue {
    guard let runtime else {
      JS.runtimeLostFatalError()
    }
   ...
   */
  @discardableResult
  public func call<each T: JSRepresentable>(this: borrowing JavaScriptObject, arguments: repeat each T) -> JavaScriptValue {
    guard let runtime else {
      JS.runtimeLostFatalError()
    }
   ...
   */
  @discardableResult
  public func call<each T: JSRepresentable>(arguments: repeat each T) -> JavaScriptValue {
    guard let runtime else {
      JS.runtimeLostFatalError()
    }
    let argumentsBuffer = JSValuesBuffer.allocate(in: runtime, with: repeat each arguments)
    return self.call(arguments: argumentsBuffer)
  }
