...
inline jsi::Value evaluateJavaScript(jsi::Runtime &runtime, const std::shared_ptr<const jsi::Buffer>& buffer, const std::string& sourceURL) {
  try {
    ...
    return jsi::Value(runtime, e.value());
  } catch (jsi::JSIException &e) {
    return jsi::Value(runtime, jsi::JSError(runtime, e.what()).value());
  } catch (...) {
    return jsi::Value(runtime, jsi::JSError(runtime, "Unknown C++ error").value());
  }
}
